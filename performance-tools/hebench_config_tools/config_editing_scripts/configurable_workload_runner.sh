...
if [[ $# -lt 4 ]]; then
        ...
        echo "Script requires the following arguments: TEST_HARNESS BACKEND CONFIG OUTPUT_FOLDER [TEST_TYPE] [SCHEME] [CATEGORY] [OTHER] [SECURITY] [CIPHER_FLAGS]" >&2
        exit 2
fi
			...
			do
				...
				do
					...
					do
						...
						$TEST_HARNESS -b $BACKEND -c $GEN_CONFIG_FILE_NAME --single_path_report --output_dir $OUTPUT_FOLDER/output_results | tee -a $OUTPUT_FOLDER/run.log
					done
				done
			done
		done
