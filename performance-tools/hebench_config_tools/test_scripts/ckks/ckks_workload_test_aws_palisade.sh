...
if [[ $# -ne 4 ]]; then
	CONFIG="$OUTPUT_FOLDER/generated_configs/default_config_$(basename $BACK_END).yml"
	$TEST_HARNESS -b $BACK_END -c $CONFIG --dump
fi

MODIFIED_CONFIG="$OUTPUT_FOLDER/generated_configs/modified_$(basename $CONFIG)"
...
$CONFIG_TOOL_DIR/configurable_env_workload_runner.sh $TEST_HARNESS $BACK_END $MODIFIED_CONFIG $OUTPUT_FOLDER 2,3 CKKS Latency,Offline ALL ALL all_cipher

export HEBENCH_MIN_TEST_TIME=10000
$CONFIG_TOOL_DIR/configurable_env_workload_runner.sh $TEST_HARNESS $BACK_END $MODIFIED_CONFIG $OUTPUT_FOLDER 4 CKKS Latency,Offline ALL ALL all_cipher

export HEBENCH_N=242
export HEBENCH_COEFF_MOD_BITS=45
export HEBENCH_SCALE_BITS=45
export HEBENCH_POLYMOD_DEGREE=16384
export HEBENCH_MULT_DEPTH=5
export HEBENCH_SAMPLE_SIZE_2=1152
$CONFIG_TOOL_DIR/configurable_env_workload_runner.sh $TEST_HARNESS $BACK_END $MODIFIED_CONFIG $OUTPUT_FOLDER 6 CKKS Latency,Offline ALL ALL all_cipher
